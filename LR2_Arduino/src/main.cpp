...
Led led(9); // 0
LightSensor lightSensor(A0); // 28
MoveSensor moveSensor(A5); // 16

IStratagy* stratagy = StrategyFactory::createStrategy("comfort");
LightingConroller controller(&led, &lightSensor, &moveSensor, stratagy);
